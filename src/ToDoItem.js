...
function ToDoItem(props) {
    ...
    const [editModeInput, setEditModeInput] = useState(props.name)
    ...
    const saveEdited = () => {
        props.saveEdited(props.id, editModeInput)
        setEditMode(false)
    }
    return (
        <div clasName='app'>
            ...
            {editMode ? (
                <div>
                    <input className="list-group-placeholder"
                           placeholder={editModeInput}
                           onChange={editModeInput}/>
                ...
                <div>
                    ...
                    {props.name}
                        ...
                        >{deleteIcon}</button>
                        <button className="btn" onClick={() => setEditMode(true)}>{editIcon}</button>
                </span>
                </div>
            )}
        </div>
...
const mapDispatchToProps = (dispatch) => ({
    ...
    saveEdited: (id, editModeInput) => dispatch({type: 'EDIT_TASK', payload: id, editModeInput})
